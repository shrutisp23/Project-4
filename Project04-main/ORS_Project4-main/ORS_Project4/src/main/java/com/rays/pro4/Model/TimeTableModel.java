...
public class TimeTableModel {
	...
	public Integer nextPK() throws DatabaseException {
		...
		try {
			...
			PreparedStatement pstmt = conn.prepareStatement("SELECT MAX(ID) FROM st_timeline");
...
//		
		...
		try {
			...
			PreparedStatement pstmt = conn.prepareStatement("INSERT st_timeline values(?,?,?,?,?,?,?,?,?,?,?,?)");
			pstmt.setInt(1, pk);
...
//			throw new ApplicationException("Exception : Exception in add timetable");
	...
	public void delete(TimeTableBean bean) throws ApplicationException {
		...
		try {
			conn = JDBCDataSource.getConnection();
			conn.setAutoCommit(false);
			PreparedStatement pstmt = conn.prepareStatement("delete from st_timeline where ID=?");
	...
	public void update(TimeTableBean bean) throws ApplicationException, DuplicateRecordException {
		...
		try {
			...
			PreparedStatement pstmt = conn.prepareStatement(
					"update st_timeline set course_id=?,course_name=?,subject_id=?,subject_name=?,semester=?,exam_date=?,exam_time=?,created_by=?,modified_by=?,created_datetime=?,modified_datetime=? where ID=?");
	...
	public TimeTableBean findByPK(long pk) throws ApplicationException {
		...
		StringBuffer sql = new StringBuffer("select * from st_timeline where id=?");
	...
	public List list(int pageNo, int pageSize) throws Exception {
		...
		StringBuffer sql = new StringBuffer("select * from st_timeline");
	...
	public List search(TimeTableBean bean, int pageNo, int pageSize) throws ApplicationException {
		...
		StringBuffer sql = new StringBuffer("select * from st_timeline where 1=1 ");
		if (bean != null) {
			if (bean.getId() > 0) {
	...
	public TimeTableBean checkBycss(long CourseId, long SubjectId, String semester) throws ApplicationException {
		...
		StringBuffer sql = new StringBuffer(
				"SELECT * FROM st_timeline WHERE Course_ID=? AND Subject_ID=? AND Semester=?");
	...
	public TimeTableBean checkBycds(long CourseId, String Semester, Date ExamDate) throws ApplicationException {
		StringBuffer sql = new StringBuffer(
				"SELECT * FROM st_timeline WHERE Course_Id=? AND semester=? AND Exam_Date=?");
